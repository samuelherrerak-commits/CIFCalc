...
const Products = {
  async render(app) {
      ...
      tbody.innerHTML = list.length === 0
        ...
        : list.map(p => `
          <tr class="border-b border-slate-100 hover:bg-slate-50">
            ...
            <td class="p-2 text-right">${fmtInt(p.qty)}</td>
    ...
    const openForm = (id = null) => {
      ...
      document.getElementById('f-qty').value = p.qty != null ? p.qty : 100;
      ...
      document.getElementById('f-margin').value = p.gain_margin != null ? p.gain_margin : 0;
    ...
    const saveProduct = () => {
      ...
      const data = {
        ...
        qty: num(document.getElementById('f-qty')),
        ...
        gain_margin: num(document.getElementById('f-margin'))
      };
      if (editingId) {
    ...
    app.innerHTML = `
      <header class="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-6 rounded-xl shadow-sm border border-slate-200 gap-4">
        <div>
          <h1 class="text-2xl font-bold text-slate-900">CIFCalc — Catálogo de Productos</h1>
        ...
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl p-5 space-y-4 max-h-[90vh] overflow-y-auto">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div>
              ...
              <input id="f-margin" type="number" min="0" step="0.1" class="w-full p-2 border rounded-lg text-sm bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none">
            </div>
          </div>
          <div>
